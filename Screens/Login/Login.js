...
import { TextInput , onChangeText } from 'react-native-gesture-handler';
     ...
     setTimeout(() => {
      ...
      if(email.trim() ==""|| password.trim() =="" ){
        setTimeout(() => {
          Toast.show({
            type:'error',
            text1:'الرجاء ادخال البيانات بشكل صحيح'
          })
         },200 ) ; return
      }else{
    await axios.post('http://172.20.10.6:5000/LoginUser' ,
    {
      email:email,
      password:password
    }).then(async(res) => {
...
<View>
  ...
  <Text>
    ...
    <View style={{
      ...
      marginRight:6,
      marginLeft:6}}>
      <TextInput placeholder='اسم المستخدم' style={{padding: 10 ,marginStart:"auto"}}
    ...
    <View style={{
      ...
      <Text style={{color:'red' , fontWeight:'700',marginRight:"auto"}}> نسيت؟ </Text>
    </TouchableOpacity>
...
<View>
  <Button title='تسجيل' color="#000" onPress={LoginUser} />
</View>

  <View style={{flexDirection :'row' ,justifyContent:'center'}}>
    <TouchableOpacity onPress={()=>navigation.navigate('SignUp')}>
      <Text style={{textAlign:'center' , color:'#ccc' , fontSize:12}}>انضم إلينا</Text>
    </TouchableOpacity>
      <Text  style={{textAlign:'center' , color:'#000' , fontSize:12}}>ليس لديك حساب؟</Text>
  </View>
